...
function Following() {
  ...
  const { following } = React.useContext(FollowContext);
  const { detail, setDetail } = React.useContext(AuthContext);
  ...
  const listItems = following.map((element) => (
    <MainDiv>
      <NameDiv>
  ...
  return (
    <>
      <TopBar text={following.length + " seguidos"} />
      {listItems}
      <BottomSpacerDiv />
      <BottomBar
        PropHome={false}
